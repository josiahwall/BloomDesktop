...
const DecodableReaderSetupDialogLauncher: React.FunctionComponent<{
    ...
    const initialSettings = useRef<ReaderSettings | undefined>(undefined);
    ...
    const save = () => {
        ...
        if (!toolboxBundle || !settings || !initialSettings.current) {
            throw new Error(
    ...
    const updateSettings = (updatedSettings: ReaderSettings) => {
        if (!initialSettings.current) {
            initialSettings.current = updatedSettings;
        }
